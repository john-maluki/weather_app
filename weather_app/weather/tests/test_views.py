...
class TestViews(TestCase):
    def setUp(self):
        ...
        self.project = City.objects.create(
            name = 'mombasa'
        )
        # self.project = mommy.make(City, name= 'mombasa')
        
        
    @patch('weather.views.index')
    def test_index_get(self, mock_index):
        response = self.client.get(self.home, data={
            'name' : 'mombasa'
        })
    ...
    @patch('weather.views.index')
    def test_index_post_ne(self, mock_index):
        response = self.client.post(self.home, {
            'name' : 'h'
        })
        self.assertEqual(response.status_code, 200)    
        
    @patch('weather.views.delete_city')
    def test_delete_city(self, mock_delete_city):
        city =mommy.make(City, name="mumbai")
